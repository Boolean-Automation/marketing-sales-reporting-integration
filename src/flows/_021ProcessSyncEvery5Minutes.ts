...
export const _021ProcessSyncEvery5Minutes = flow({
  ...
  onExecution: async (context, params) => {
    ...
    const time2MinutesAgoToEpoch =
      await context.components.datetime.convertToEpoch({
        dateTimeInput: adjustDateTime.data,
  ...
  const access_token = stepResults?.crossFlow?.results?.body?.data?.hubspotTokenId;
...
`,
    ...
    for (const loopOverItemsItem of generateGetDealPayload.data.payload) {
...
"config":${params.onTrigger.results.body.data}
      ...
      loopOverItems.data.push(invokeFlow022.data);
    }
...
module.exports = async ({ logger, configVars }, stepResults) => {
  try {
    ...
    const access_token = data?.hubspotTokenId;
    ...
    logger.info(\`🗑️ Deleted \${rowsToDelete.length} rows for Deal IDs found in archived deals.\`);
