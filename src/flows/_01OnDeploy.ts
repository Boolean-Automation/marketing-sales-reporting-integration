...
`,
    ...
    for (const loopOverItemsItem of (getConfigVariables as { data: unknown[] }).data) {
      const processMassImport = await context.components.crossFlow.invokeFlow({
        data: loopOverItemsItem as string | undefined,
        flowName: "01.1 Process Mass Syncing",
      });

      loopOverItems.data.push((processMassImport as { data: unknown }).data);
    }
